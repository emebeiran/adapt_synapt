...
for i, gS in enumerate([ 0.5]):#, 0.0005,0.005,([0.2,0.02, 0.002, 0.0002]):
    ...
    borderT = gS+np.sqrt(2*gS*(gS+1))#np.sqrt(2*gS/(1+gS))
...
if len(xx)>0:
    xx = xx[-1]
    yy = yy[-1]
    ax.scatter([xx], [yy], 20,  facecolors='k', edgecolors= [1, 1, 1], zorder=20)
...
gborder = -1-frac+np.sqrt(1+2*frac)
...
gborder = -1-frac+np.sqrt(1+2*frac)#frac**2/(2-frac**2)
for i, f in enumerate(fracs[:-1]):
    x = st_osc[i]
    x1 = x[gS<gborder]
    x2 = x[gS>gborder]
...
gaus = np.exp(-x**2/(2*3**2))
ex = np.exp(-np.abs(x)yfx/3.)
gaus = gaus/np.max(gaus)
plt.plot(x, gaus)
plt.plot(x, ex/np.max(ex))
plt.plot(x,(1./np.sqrt(np.e))*np.ones(len(x)))
fake = np.argsort(np.abs(gaus-(1./np.sqrt(np.e))))
dist = (x[1]-x[0])*(fake[1]-fake[0])