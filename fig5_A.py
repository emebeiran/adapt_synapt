...
fig = plt.figure()
for ifact, fact in enumerate([0.95,1.09,  1.175, 1.4]):# 1.09, 1.175,1.4]): #1.4
    ...
    try:     
        fl = np.load("sigmamatrix_data_J"+str(fact)+"_sigma_"+str(sigma)+".npz")
        Jmat = fl['name1']
        w = fl['name2']
        sol = fl['name3']
        sol2 = fl['name4']
        ...
        tau = 1.
        tau_w = 5.
        sol, sol2, meanr, meana, q1, q2 = ll.rate_simnoisefinal(time, xws0, Jmat, tau, tau_w, g_w, phimax, sigma, method='rk4', numbs =2*int(N))
        #sol, sol2, meanr, meana, q1, q2 = ll.rate_simfinal(time, xws0, Jmat, tau, tau_w, g_w, phimax,  method='rk4', numbs =2*int(N))
        #
        np.savez("sigmamatrix_data_J"+str(fact)+"_sigma_"+str(sigma), name1 = Jmat, name2 = w, name3= sol, name4 = sol2)
