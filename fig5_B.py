...
for ifact, fact in enumerate([0.95, 1.09, 1.175, 1.43]):#[0.95, 1.09, 1.175, 1.43]):#[0.95, 1.09, 1.175, 1.43]):
    ...
    try:     
        fl = np.load("1sigmanewsynmatrix_data_J"+str(fact)+"_sigma_"+str(sigma)+".npz")
        Jmat = fl['name1']
        w = fl['name2']
        sol = fl['name3']
        sol2 = fl['name4']
        ...
        tau = 1.
        tau_w = 5.
        sol, sol2, meanr, meana, q1, q2 = ll.rate_simfinalnoise_adap(time, xws0, Jmat, tau, tau_w, g_w, phimax, sigma, method='rk4', numbs =2*int(N))
        np.savez("sigmanewsynmatrix_data_J"+str(fact)+"_sigma_"+str(sigma), name1 = Jmat, name2 = w, name3= sol, name4 = sol2)
    ...
    for ind, n in enumerate(range(nums)):
        if ifact == 0:
            colors[:,n] = 0.6*color1 + 0.4*(np.random.rand(3))
    ...
    a.scatter(np.real(w), np.imag(w), 0.05, 0.01*color1+0.99*0.6*np.ones(3))
    ...
    for i in range(len(a1[0])+1):
        if i == 0 :
            ...
            idx_0= a1[0][i-1]+1
            idx_fin = a1[0][i]-1
        a.plot(np.real(lambda1[idx_0:idx_fin]), np.imag(lambda1[idx_0:idx_fin]),  color='k', lw = 0.5)
    ...
    for i in range(len(a1[0])+1):
        if i == 0 :
            ...
            idx_0= a1[0][i-1]+1
            idx_fin = a1[0][i]-1
        a.plot(np.real(lambda2[idx_0:idx_fin]), np.imag(lambda2[idx_0:idx_fin]),  color='k', lw = 0.5)
    ...
    a.set_xlim(XL)
    a.set_ylim([-0.5, 0.5])
    a.text(-0.2, 0.35, '$Im(\lambda)$', fontsize=5)
    a.text(0.08, 0.05, '$Re(\lambda)$', fontsize=5)
    
    a.plot([-5,5],[0,0],'k', lw = 0.2)
    a.plot([0,0],[-4,5], 'k', lw = 0.2)
    a.axis('off')
plt.savefig('adap_fig5b.pdf')
